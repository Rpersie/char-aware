...
import model
from data_reader_cn import load_data, DataReader


flags = tf.flags

# data
flags.DEFINE_string('data_dir',    'data',   'data directory. Should contain train.txt.txt/valid.txt.txt/train.txt.txt with input data')
...
flags.DEFINE_integer('batch_size',          20,   'number of sequences to train.txt on in parallel')
flags.DEFINE_integer('max_epochs',          25,   'number of full passes through the training data')
flags.DEFINE_float  ('max_grad_norm',       5.0,  'normalize gradients at')
flags.DEFINE_integer('max_word_length',     200,   'maximum word length')
...
def main(_):
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        summary_loss=tf.summary.scalar("loss",train_model.loss)
        summary_ppl=tf.summary.scalar("perplexity",tf.exp(train_model.loss))
        ...
        for epoch in range(FLAGS.max_epochs):
            ...
            for x, y in train_reader.iter():
                ...
                loss, _, rnn_state, gradient_norm, step, _,s = session.run([
                    train_model.loss,
                ...
                summary_writer.add_summary(s,epoch*train_reader.batch_size+count)
            ...
            print("train.txt loss = %6.8f, perplexity = %6.8f" % (avg_train_loss, np.exp(avg_train_loss)))
