...
def main(_):
    ''' Loads trained model and evaluates it on test.txt split '''
    ...
    test_reader = DataReader(word_tensors['test.txt'], char_tensors['test.txt'],
                              FLAGS.batch_size, FLAGS.num_unroll_steps)

    print('initialized test.txt dataset reader')
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        print("test.txt loss = %6.8f, perplexity = %6.8f" % (avg_loss, np.exp(avg_loss)))
        print("test.txt samples:", count*FLAGS.batch_size, "time elapsed:", time_elapsed, "time per one batch:", time_elapsed/count)
